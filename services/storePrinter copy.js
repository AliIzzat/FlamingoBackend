...
//   if (!store.printer || !store.printer.enabled) {
//     throw new Error(`Printer is not enabled for store: ${store.name}`);
//   }

//   if (!store.printer.ipAddress) {
//     throw new Error(`Printer IP is missing for store: ${store.name}`);
//   }

//   const port = Number(store?.printer?.port || 9100);
//   const ticketBuffer = buildTicketBuffer(order, store);

//   await sendRawToPrinter(store.printer.ipAddress, port, ticketBuffer);

//   return {
//     success: true,
//     storeId: String(store._id),
//     storeName: store.name,
//     printerIp: store.printer.ipAddress,
//     printerPort: port,
//   };
// }
