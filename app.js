...
app.get("/seed-notifications", async (req, res) => {
  try {
    ...
    for (const order of orders) {
      ...
      if (order.delivery?.status === "Claimed") {
        status = "claimed";
        driverId = driver?._id || null;
        message = `Order claimed by driver`;
      }

      if (order.delivery?.status === "PickedUp") {
        status = "picked";
        driverId = driver?._id || null;
