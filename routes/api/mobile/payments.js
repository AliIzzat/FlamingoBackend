...
router.post("/myfatoorah/initiate", async (req, res) => {
  try {
    ...
    const { total } = await saveOrderTotals(order, totalAmount);
    const methodId = Number(paymentMethodId || 2);
    const baseUrl = getPublicBaseUrl();
    const returnUrl = `${baseUrl}/api/mobile/payments/myfatoorah/return?orderId=${orderId}`;
...
router.get("/myfatoorah/return", async (req, res) => {
  ...
  try {
    ...
    if (!paymentId) {
      ...
      invoiceId = order?.payment?.invoiceId ? String(order.payment.invoiceId) : "";
    }
    ...
    await finalizePaidOrder(orderId, verification, { invoiceId, paymentId });
    ...
    return res.status(200).send(
      renderReturnPage({
        ...
        orderId,
        paymentId:
          String(verification?.tx?.PaymentId || paymentId || "-"),
        deepLink: deepLinkReturn({
          orderId,
          paymentId: String(verification?.tx?.PaymentId || paymentId || ""),
          status: verification.invoiceStatus,
        }),
        note: `Verified via ${verification.keyType}`,
      })
    );
  } catch (err) {
