...
// ------------------------
const MF_BASE = (process.env.MYFATOORAH_API_URL || "https://apitest.myfatoorah.com")
  .trim()
  .replace(/\/+$/, "")     // remove trailing slashes
  .replace(/\/v2$/i, "");  // remove trailing /v2 if someone put it in env

const MF_TOKEN = (process.env.MYFATOORAH_TOKEN || "").trim();
...
router.post("/myfatoorah/initiate", async (req, res) => {
  try {
    ...
    console.log("✅ HIT /myfatoorah/initiate");
    ...
    if (!MF_TOKEN) {
      return res.status(500).json({
        ok: false,
        error: "MYFATOORAH_TOKEN is missing on server",
      });
    }
    ...
    console.log("✅ MF ExecutePayment:", r.data?.Message, "IsSuccess:", r.data?.IsSuccess);
    console.log("✅ PaymentURL:", data?.PaymentURL);

    // Save invoiceId on the order
    if (data?.InvoiceId) {
    ...
    console.error("❌ ExecutePayment FAILED status:", status);
    console.error("❌ ExecutePayment FAILED details:", details);
